...
const ChartComponent = ({ chartType }) => {
  ...
  useEffect(() => {
    if (graphData) {
      setDashboardIncome(calculateTotalIncome(graphData.data));
      setDashboardExpense(calculateTotalExpense(graphData.data));
      setDashboardBalance(dashboardIncome - dashboardExpense);
      setChartData(
        graphData
          ? convertGroupedDataToArray(groupDataByCategory(graphData.data))
          : []
      );
    }
  ...
  useEffect(() => {
    setDashboardBalance(dashboardIncome - dashboardExpense);
  }, [dashboardIncome, dashboardExpense]); //everytime dashboardIncome or dashboardExpense is updated, dashboardBalance is recalculated
            ...
            return (
              <ScatterChart width={500} height={300}>
                ...
                <Scatter
                  ...
                  data={
                    graphData &&
                    graphData.data.filter((entry) => entry.type === "income")
                  }
                  fill="#00C49F"
                />
                <Scatter
                  ...
                  data={
                    graphData &&
                    graphData.data.filter((entry) => entry.type === "expense")
                  }
                  fill="#FF8042"
                />
                <Legend />
